...
function buildSettleData(ym){
  ...
  forEachVerifiedLog((l,m)=>{
    ...
    byStf[key][bucket].push({
      ...
      stu: (IDX.stuById[m.stuId]||{}).nm || '',
      sc:  (IDX.stuById[m.stuId]||{}).sc || ''
    });
  }, {ym});
  // 연수 출장비
  (db.trn||[]).forEach(t=>{
...
function exportExecReportXlsx(){
  ...
  const aoa = [[`월별 활동비 집행내역서 (${ym})`],
    [`기관: ${db.cfg.org||''}`],
    [],['No','지원단','코칭회수','코칭금액','협력회수','협력금액','출장회수','출장금액','총액(세전)']];
  let g1=0,g2=0,g3=0;
  ids.forEach((id,i)=>{
    const d = data[id]; const stf = (IDX.stfById||{})[id];
    ...
    aoa.push([i+1, stf?stf.nm:'?', d.coach.length, c1, d.cls.length, c2, d.travel.length, c3, c1+c2+c3]);
    g1+=c1; g2+=c2; g3+=c3;
  });
  aoa.push([]);
  ...
  const wb = XLSX.utils.book_new();
  const ws = XLSX.utils.aoa_to_sheet(aoa);
  ws['!cols'] = [{wch:5},{wch:12},{wch:10},{wch:14},{wch:10},{wch:14},{wch:10},{wch:14},{wch:14}];
...
window.goTab = function(id, btn){
  ...
  if(id==='t5'){ if(!$('rec-date').value) $('rec-date').value = todayStr(); }
...
window.addEventListener('load', ()=>{
  setTimeout(()=>{
    loadRatesAndBudget();
    if($('rec-date') && !$('rec-date').value) $('rec-date').value = todayStr();
  }, 800);
});
