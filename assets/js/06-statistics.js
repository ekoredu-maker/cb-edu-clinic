...
function pivotByGrade(){
  ...
  db.stu.forEach(s=>{
    ...
    if(sts.includes('방과후학습코칭')){ res[key].apply_coach++; res[key].coach++; }
    if(sts.includes('수업협력코칭')){ res[key].apply_class_stu++; res[key].class_stu++; }
...
function pivotByRegion(){
  ...
  db.stu.forEach(s=>{
    ...
    if(!res[r]) return;
    const sts = s.supportTypes||[];
    if(sts.includes('방과후학습코칭')){ res[r].apply_coach++; res[r].coach++; }
    if(sts.includes('수업협력코칭')){ res[r].apply_class_stu++; res[r].class_stu++; }
